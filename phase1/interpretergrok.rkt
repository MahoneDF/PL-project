...
(define process-declarations
  (lambda (decl-list env)
    (displayln decl-list)
    (displayln env)
    (displayln (car decl-list))
    (displayln "1")
    (displayln (value-of-declaration (car decl-list) env))
    (displayln "2")
    (if (null? decl-list)
        env
...
(define value-of-declaration
  (lambda (decl env)
    (cases declaration decl
      ...
      (fundec-decl (fun-decl) (displayln "3") (displayln "4") (displayln fun-decl) (value-of-fun-declaration fun-decl env) (displayln "5")))))
    ...
    (cases fun-declaration fun-decl
      (fun-dcl (type-spec id params body)
               (let ((param-names (extract-param-names params)))
                 ...
                 (displayln params)
                 (displayln body)
                 (displayln param-names)
                 (displayln (newref (proc-val (procedure param-names body env))))
                 (displayln (number? (newref (proc-val (procedure param-names body env)))))
                 (extend-env id
